    ...
    for episode in range(num_episodes):
      ...
      while True:
        ...
        if (done):
          ...
          episode_val_target = (episode_val_target - np.mean(episode_val_target)) / (np.std(episode_val_target) + eps)
          break
...
class Policy_Gradient():
  ...
  def pg_train(self,training_num):
    ...
    for i in range(training_num):
      ...
      train_state, train_actions, train_vt,rewards,running_rewards,train_log = sampler.sample_episode(self.actor_net,1)
      self.actor_net.update_policy(train_vt,train_log)
      print("\repisode:{} |reward: {}".format(i, rewards[-1]), end="")
      training_time.append(i)
      reward_sum_line.append(rewards[-1])
...
if __name__=='__main__':
  ...
  if args.run_mode=='test':
    ...
    pg = Policy_Gradient(actor_net,env, lr=0.01)
    pg.pg_train(args.train_num)
